...
class BiliTaskManager():
    ...
    def get_up_check_update_list(self) -> List[List[str]]:
        '''每次触发检查更新,

        Returns:
            List[List[str, int]]: 应检查更新的up和时间戳的列表
        '''
        check_update_list = []
        for up_uid in self.up_list.keys():
            if self.up_list[up_uid]["next_update_check"] == self.__up_update_check_ptr__:
                check_update_list.append([up_uid, self.up_list[up_uid]["latest_timestamop"]])
        self.__up_update_check_ptr__ = (self.__up_update_check_ptr__ + 1) % 5
    ...
    def update_up_info(self, up_uid: str, latest_timestamp: int) -> bool:
        ...
        try:
            ...
            self.up_list[up_uid]["latest_timestamop"] = latest_timestamp
            return True
